...
public class ReflectionUtils {
    ...
    /**
     * Determine if the passed in class (classToCheck) has the annotation (annoClass) on itself, any of its super
     * classes, any of it's interfaces, or any of it's super interfaces. This is a exhaustive check throughout the
     ...
     *     fields).  This makes field traversal on a class faster as it does not need to continually process known
     *     fields like primitives.
     */
     ...
     *
     * @param c Class instance that would need further processing (reference fields).  This makes field traversal on a
     *          class faster as it does not need to continually process known fields like primitives.
     */
    ...
    public static Object call(Object bean, Method method, Object... args) {
        if (method == null) {
            ...
            throw new IllegalArgumentException("null Method passed to ReflectionUtils.call() on bean of type: "
                                               + className);
        }
        if (bean == null) {
        ...
        try {
            ...
            throw new RuntimeException("IllegalAccessException occurred attempting to reflectively call method: "
                                       + method.getName()
                                       + "()", e);
        } catch (InvocationTargetException e) {
            throw new RuntimeException("Exception thrown inside reflectively called method: "
                                       + method.getName()
    ...
    /**
     * Make a reflective method call in one step.  This approach does not support calling two different methods with the
     * same argument count, since it caches methods internally by "className.methodName|argCount".  For example, if you
     * had a class with two methods, foo(int, String) and foo(String, String), you cannot use this method. However, this
     * method would support calling foo(int), foo(int, String), foo(int, String, Object), etc. Internally, it is caching
     * the reflective method lookups as mentioned earlier for speed, using argument count as part of the key (not all
     * argument types).
     * <p>
     * Ideally, use the call(Object, Method, Object...args) method when possible, as it will support any method, and
     * also provides caching.  There are times, however, when all that is passed in (REST APIs) is argument values, and
     * if some of those are null, you may have an ambiguous targeted method.  With this approach, you can still call
     * these methods, assuming the methods are not overloaded with the same number of arguments and differing types.
     *
     * @param bean       Object instance on which to call method.
     ...
     * instead of reflectively searching for the method every time.  Ideally, use the other getMethod() API that takes
     * an additional argument, Class[] of argument types (most desirable). This is to better support overloaded methods.
     * Sometimes, you only have the argument values, and if they can be null, you cannot call the getMethod() API that
     * take argument Class types.
     *
     ...
     * @param argCount   int number of arguments.  This is used as part of the cache key to allow for duplicate method
     *                   names as long as the argument list length is different.
    ...
    public static Method getMethod(Object bean, String methodName, int argCount) {
        ...
        if (methodName == null) {
            throw new IllegalArgumentException(
                "Attempted to call getMethod() with a null method name on an instance of: "
    ...
    /**
     * Fetch the named method from the passed in Class. This method caches found methods, so it should be used instead
     * of reflectively searching for the method every time.  This method expects the desired method name to not be
     * overloaded.
     *
    ...
    public static Method getNonOverloadedMethod(Class clazz, String methodName) {
        ...
        if (methodName == null) {
            throw new IllegalArgumentException("Attempted to call getMethod() with a null method name on class: "
                                               + clazz.getName());
        }
        ...
        for (Method method : methods) {
            if (methodName.equals(method.getName())) {
                if (foundMethod != null) {
                    throw new IllegalArgumentException("Method: "
                                                       + methodName
                                                       + "() called on a class with overloaded methods - ambiguous as to which one to return.  Use getMethod() that takes argument types or argument count.");
                }
                foundMethod = method;
            }
        }
        ...
        for (int i = 0; i < cpcnt; i++) {
            ...
            } else if (t == 8) // CONSTANT_String
            {
                dis.readShort(); // string_index
                       ...
                       || t == 10
                       || t == 11)  // CONSTANT_Fieldref || CONSTANT_Methodref || CONSTANT_InterfaceMethodref
            {
