...
import static com.cloud.arch.aggregate.reflection.Converter.convert2BigDecimal;
import static com.cloud.arch.aggregate.reflection.Converter.convert2boolean;
import static com.cloud.arch.aggregate.reflection.ReflectionUtils.getClassLoaderName;
...
public class DeepEquals {
    ...
    /**
     * Compare two objects with a 'deep' comparison.  This will traverse the Object graph and perform either a
     * field-by-field comparison on each object (if not .equals() method has been overridden from Object), or it will
     * call the customized .equals() method if it exists.  This method will allow object graphs loaded at different
     * times (with different object ids) to be reliably compared.  Object.equals() / Object.hashCode() rely on the
     * object's identity, which would not consider to equivalent objects necessarily equals.  This allows graphs
     * containing instances of Classes that did no overide .equals() / .hashCode() to be compared.  For example, testing
     * for existence in a cache.  Relying on an objects identity will not locate an object in cache, yet relying on it
     * being equivalent will.<br><br>
     * <p>
     * This method will handle cycles correctly, for example A-&gt;B-&gt;C-&gt;A.  Suppose a and a' are two separate
     * instances of the A with the same values for all fields on A, B, and C.  Then a.deepEquals(a') will return true.
     * It uses cycle detection storing visited objects in a Set to prevent endless loops.
     *
     * @param a Object one to compare
     * @param b Object two to compare
     * @return true if a is equivalent to b, false otherwise.  Equivalent means that all field values of both subgraphs
     *     are the same, either at the field level or via the respectively encountered overridden .equals() methods
     *     during traversal.
     */
     ...
     * @param a       Object one to compare
     * @param b       Object two to compare
     * @param options Map options for compare. With no option, if a custom equals() method is present, it will be used.
     *                If IGNORE_CUSTOM_EQUALS is present, it will be expected to be a Set of classes to ignore. It is a
     *                black-list of classes that will not be compared using .equals() even if the classes have a custom
     *                .equals() method present.  If it is and empty set, then no custom .equals() methods will be
     *                called.
     * @return true if a is equivalent to b, false otherwise.  Equivalent means that all field values of both subgraphs
     *     are the same, either at the field level or via the respectively encountered overridden .equals() methods
     *     during traversal.
    ...
    private static boolean deepEquals(Object a, Object b, Map<?, ?> options, Set<ItemsToCompare> visited) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (key1 == null
                || key2
                   == null) {   // If either one is null, they are not equal (both can't be null, due to above comparison).
                return false;
            }
            ...
            if (key1 instanceof Number
                || key2 instanceof Number) {   // If one is a Number and the other one is not, then optionally compare them as strings, otherwise return false
            ...
            if (key1Class.isPrimitive()
                || prims.contains(key1Class)
                || key1 instanceof String
                || key1 instanceof Date
                || key1 instanceof Class) {
                if (!key1.equals(key2)) {
                    return false;
                }
            ...
            if (!isContainerType(key1)
                && !isContainerType(key2)
                && !key1Class.equals(key2.getClass())) {   // Must be same class
                return false;
            }
            ...
            if (hasCustomEquals(key1Class)) {
                if (ignoreCustomEquals == null || (ignoreCustomEquals.size() > 0
                                                   && !ignoreCustomEquals.contains(key1Class))) {
                    if (!key1.equals(key2)) {
                        return false;
                    }
                    continue;
    ...
    /**
     * Deeply compare to Arrays []. Both arrays must be of the same type, same length, and all elements within the
     * arrays must be deeply equal in order to return true.
     *
     ...
     * @param stack   add items to compare to the Stack (Stack versus recursion)
     * @param visited Set of objects already compared (prevents cycles) value of 'true' indicates that the Collections
     *                may be equal, and the sets items will be added to the Stack for further comparison.
     */
    ...
    /**
     * Deeply compare the two sets referenced by ItemsToCompare.  This method attempts to quickly determine inequality
     * by length, then if lengths match, it places one collection into a temporary Map by deepHashCode(), so that it can
    ...
    private static boolean compareUnorderedCollection(Collection col1, Collection col2, Deque stack, Set visited,
        ...
        for (Object o : col1) {
            ...
            if (other == null
                || other.isEmpty()) {   // fail fast: item not even found in other Collection, no need to continue.
                return false;
            }
    ...
    /**
     * Deeply compare two SortedMap instances.  This method walks the Maps in order, taking advantage of the fact that
     * the Maps are SortedMaps.
     *
     ...
     * @return false if the Maps are for certain not equals.  'true' indicates that 'on the surface' the maps are equal,
     *     however, it will place the contents of the Maps on the stack for further comparisons.
     */
    ...
    /**
     * Deeply compare two Map instances.  After quick short-circuit tests, this method uses a temporary Map so that this
     * method can run in O(N) time.
     *
     * @param map1    Map one
     * @param map2    Map two
     * @param stack   add items to compare to the Stack (Stack versus recursion)
    ...
    /**
     * @return true of the passed in o is within the passed in Collection, using a deepEquals comparison element by
     *     element.  Used only for hash collisions.
     */
    ...
    /**
     * Get a deterministic hashCode (int) value for an Object, regardless of when it was created or where it was loaded
     * into memory.  The problem with java.lang.Object.hashCode() is that it essentially relies on memory location of an
     * object (what identity it was assigned), whereas this method will produce the same hashCode for any object graph,
     * regardless of how many times it is created.<br><br>
     * <p>
     ...
     *
     * @param c Class to check.
     * @return true, if the passed in Class has a .hashCode() method somewhere between itself and just below Object in
     *     it's inheritance.
     */
