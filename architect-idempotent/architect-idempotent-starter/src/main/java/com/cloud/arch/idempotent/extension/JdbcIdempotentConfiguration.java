...
import com.cloud.arch.idempotent.IdemCleanupScheduler;
import com.cloud.arch.idempotent.JdbcIdempotentManager;
import com.cloud.arch.idempotent.IdempotentProperties;
...
public class JdbcIdempotentConfiguration {
    ...
    @Bean
    @ConditionalOnBean(value = {DataSource.class, MutexTemplate.class})
    @ConditionalOnProperty(prefix = "com.cloud.idempotent.mysql.cleanup", name = "enabled", havingValue = "true")
    public IdemCleanupScheduler cleanupTask(DataSource dataSource, IdempotentProperties properties, MutexTemplate mutexTemplate) {
        return new IdemCleanupScheduler(dataSource, mutexTemplate, properties);
    }
